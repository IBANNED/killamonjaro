...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	Render::Text(window->m_x + 8, window->m_y + 8, Color(240, 240, 240, 255), Render::Fonts::Slider, window->Title.c_str());
	...
	if (TabCount) // If there are some tabs
	{
		...
		for (int i = 0; i < TabCount; i++)
		{
			...
			//text tab 1
			RECT TextSize = Render::GetTextSize(Render::Fonts::Slider, tab->Title.c_str());
			Render::Text(TabArea.left + (TabSize / 3) - (TextSize.right / 2), TabArea.top + 8, Color(240, 240, 240, 255), Render::Fonts::Slider, tab->Title.c_str());
